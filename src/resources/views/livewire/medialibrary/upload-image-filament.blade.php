    ...
    <div class="{{ $selectedPicture ? 'w-2/3' : 'w-full' }} space-y-4 pb-5">
        ...
        <div x-data="{ open: false }" class="shadow-md p-8 rounded-md bg-white">
            <button x-show="!open" x-on:click="open = true"
                class="w-full py-2 px-4 bg-gray-100 border border-gray-300 hover:bg-gray-200 text-gray-800 rounded-md mt-4">
            ...
            <form x-show="open" wire:submit.prevent="save" class="space-y-4 pb-5">
                ...
                @error('photo')
                    ...
                    setTimeout(() => showError = false, 4000)" x-show="showError"
                        class="error text-red-500">{{ $message }}</span>
                @enderror
                ...
                <input type="text" wire:model.defer="altText" class="w-full py-2 px-3 border rounded-md">

                <button type="submit"
                    class="w-full py-2 px-4 bg-blue-400 border border-blue-500 hover:bg-blue-300 rounded-md mt-4"
                    wire:loading.attr="disabled" wire:target="photo" wire:loading.class="opacity-50 cursor-not-allowed">
                    Foto opslaan
                </button>
            ...
            <div class="pb-4 mt-4">
                <button wire:click="generateMissingResizedImages"
                    class="py-2 px-4 bg-blue-400 border border-blue-500 hover:bg-blue-300 rounded-md mb-2">Genereer
                    missenden resized afbeeldingen voor alles</button>
                <!-- Delete Selected Button -->
                <form wire:submit.prevent="confirmDeleteSelected">
                    @csrf
                    <button type="submit"
                        class="bg-red-400 border border-red-500 px-4 py-2 rounded-md hover:bg-red-300 focus:outline-none focus:ring-2 focus:ring-red-600 focus:ring-opacity-50">
                        Delete Selected
                    </button>
                </form>
            </div>
    ...
    @if ($selectedPicture)
        <div class="w-1/3 ml-4 rounded-md bg-white p-6" wire:key="selectedPicture_{{ $selectedPicture->id }}">
            ...
            <form wire:submit.prevent="saveTags" class="mb-4">
                ...
                <button type="submit"
                    class=" py-2 px-4 bg-blue-400 border border-blue-500 hover:bg-blue-300 rounded-md mt-4">
                    Tags opslaan
                </button>
            </form>
            ...
            <div class="mb-4">
                ...
                <button wire:click="updateFileTitle"
                    class=" py-2 px-4 bg-blue-400 border border-blue-500 hover:bg-blue-300 rounded-md mt-4">Update
                    Title</button>
            </div>

            <div class="mb-4">
                <div class="font-bold">Alt text:</div>
                @if ($selectedPicture)
                    <input type="text" wire:model.defer="altText" class="w-full py-2 px-3 border rounded-md">
                @endif
                <button wire:click="updateAltText"
                    class=" py-2 px-4 bg-blue-400 border border-blue-500 hover:bg-blue-300 rounded-md mt-4">Update Alt
                    text</button>
            </div>
            ...
            <div class="flex flex-wrap mb-4">
                @forelse ($selectedPicture->resizedImages as $resizedImage)
                    <div class="flex items-center">
                        <div class="bg-gray-200 text-gray-800 px-2 py-1 rounded-t-sm rounded-tr-sm mr-2 mb-2 sm:w-auto"
                            wire:key="selectedPictureResizedImage_{{ $resizedImage->id }}">
                            <div class="whitespace-normal break-words overflow-hidden overflow-ellipsis text-xs">
                ...
                <button wire:click="generateMissingResizedImagesForSelected"
                    class="py-2 px-4 bg-blue-400 border border-blue-500 hover:bg-blue-300 rounded-md mt-4">
                    Genereer missenden resized afbeeldingen
                </button>
            </div>
            <div>
